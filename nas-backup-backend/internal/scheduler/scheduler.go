...
// Every scheduled run executes a standalone backup — there is no
// full/incremental distinction.
package scheduler
...
// runBackup is the cron job callback that executes a backup.
func (s *Scheduler) runBackup() {
	slog.Info("scheduled backup triggered")

	ctx := context.Background()
	if err := s.engine.RunBackup(ctx); err != nil {
