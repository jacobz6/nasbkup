...
func (e *Engine) reconcileApplyFixes(ctx context.Context, report *ReconcileReport) error {
	// ── Safety pre-check: refuse to apply when deletions would be catastrophic ──
	// In a shared-bucket multi-environment setup, a fresh empty DB on a new
	// machine (or after accidental DB deletion) would list EVERY existing
	// OSS object as an "OSS-only orphan" and delete the entire bucket. To
	// prevent total data loss we detect this specific failure mode and abort
	// the entire reconcile pass (not just the OSS delete phase — the DB
	// mutations below could also be wrong on an uninitialized instance).
	//
	// Threshold: if hash_index has 0 rows AND backup_files has 0 rows AND
	// we would delete >=1 OSS object, this is almost certainly the
	// uninitialized-instance scenario; abort.
	{
		hashCount, hErr := e.db.HashRepo.CountAll()
		bfCount, bfErr := e.db.BackupRepo.CountAllBackupFiles()
		if (hErr == nil && hashCount == 0) && (bfErr == nil && bfCount == 0) && len(report.OSSOnlyOrphans) > 0 {
			msg := fmt.Sprintf(
			...
			e.logger.Error("reconcile safety guard triggered",
				"oss_orphans", len(report.OSSOnlyOrphans))
